...
export class PathWalker extends BaseScriptComponent {
  ...
  private onUpdate() {
    ...
    switch (this.state) {
      ...
      case 1: // prep
        break
      case 2: {
        if (this.isNavigationMode) break;  // 导航模式跳过所有游戏逻辑
  ...
  public start(
    ...
    if (!isNull(myFinishLineTr)) {
      this.finishLineTr = myFinishLineTr
    ...
    if (this.isNavigationMode) {
      this.currentState = PathWalkerState.Walking
      this.state = 2 // jump directly to walking
      this.timerScript.start()
      
      const targetPos = this.finishLineTr ? this.finishLineTr.getWorldPosition() : myStartLineTr.getWorldPosition()
      this.arrowSpawner.startNavigation(targetPos)
    } else {
