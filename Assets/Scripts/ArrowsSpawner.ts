...
export class ArrowsSpawner extends BaseScriptComponent {
  ...
  private navTargetPos: vec3 | null = null
  private navCompassObj: { leftArrow: SceneObject, rightArrow: SceneObject } | null = null

  start(arrowsPositions: vec3[], mySplinePoints: {position: vec3; rotation: quat}[], myPathLength: number) {
    this.clearState()
    this.navTargetPos = null
    this.nextId = 0
  ...
  startNavigation(targetPosition: vec3) {
    this.clearState()
    this.navTargetPos = targetPosition

    if (this.navCompassObj) {
      if (!isNull(this.navCompassObj.leftArrow)) this.navCompassObj.leftArrow.destroy()
      if (!isNull(this.navCompassObj.rightArrow)) this.navCompassObj.rightArrow.destroy()
    }
    
    this.navCompassObj = this.instantiateSideArrowObj(vec3.zero(), quat.quatIdentity())
    this.navCompassObj.leftArrow.getTransform().setLocalScale(new vec3(0.5, 0.5, 0.5))
    this.navCompassObj.rightArrow.getTransform().setLocalScale(new vec3(0.5, 0.5, 0.5))
